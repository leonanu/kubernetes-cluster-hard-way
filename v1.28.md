...
```shell
mv kubernetes kubernetes-1.28.13
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.28.13 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.28.13 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.28.13 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.28.13/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.28.13/node/bin/kubeadm /usr/local/bin/
```
...
* [下载](https://github.com/cri-o/cri-o/releases/tag/v1.28.10#downloads)
...
```shell
tar xzvf cri-o-v1.28.10.tar.gz
```
...
* [下载](https://github.com/containerd/containerd/releases/download/v1.7.22/containerd-1.7.22-linux-amd64.tar.gz) Containerd
...
* [下载](https://github.com/opencontainers/runc/releases/download/v1.1.14/runc.amd64) RunC
...
```shell
tar xzvf containerd-1.7.22-linux-amd64.tar.gz -C ./containerd
```
...
for I in {1..6};do
    ...
    ssh root@v${I} "mkdir -p /opt/app/containerd-1.7.22-linux-amd64/bin && ln -snf /opt/app/containerd-1.7.22-linux-amd64 /opt/containerd"
    ...
    ssh root@v${I} "mkdir -p /opt/app/runc-v1.1.14-linux-amd64/sbin && ln -snf /opt/app/runc-v1.1.14-linux-amd64 /opt/runc"
