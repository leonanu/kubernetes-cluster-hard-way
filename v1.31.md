...
| CRI-O                           | 1.31.4           |
| cfssl                           | 1.6.2            |
| Containerd （可选）             | 1.7.24            |
| CNI-Plugins （可选）            | 1.6.1            |
| crictl （可选）                 | 1.31.1           |
| RunC （可选）                   | 1.2.4           |
| etcd                            | 3.5.16           |
| CoreDNS                         | 1.8.6            |
...
```shell
mv kubernetes kubernetes-1.31.4
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.31.4 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.31.4 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.31.4 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.31.4/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.31.4/node/bin/kubeadm /usr/local/bin/
```
...
* [下载](https://github.com/cri-o/cri-o/releases/tag/v1.31.4#downloads)
...
* [下载](https://github.com/containerd/containerd/releases/download/v1.7.24/containerd-1.7.24-linux-amd64.tar.gz) Containerd

* [下载](https://github.com/containernetworking/plugins/releases/download/v1.6.1/cni-plugins-linux-amd64-v1.6.1.tgz) CNI-Plugins

* [下载](https://github.com/kubernetes-sigs/cri-tools/releases/download/v1.31.1/crictl-v1.31.1-linux-amd64.tar.gz) crictl

* [下载](https://github.com/opencontainers/runc/releases/download/v1.2.4/runc.amd64) RunC
...
```shell
tar xzvf cni-plugins-linux-amd64-v1.6.1.tgz -C ./cni-plugins
```
...
```shell
tar xzvf containerd-1.7.24-linux-amd64.tar.gz -C ./containerd
```
...
```shell
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app/cni-plugins-linux-amd64-v1.6.1/bin && ln -snf /opt/app/cni-plugins-linux-amd64-v1.6.1 /opt/cni"
    rsync -avr /usr/local/src/cni-plugins/ root@v${I}:/opt/cni/bin/
    ssh root@v${I} "chown -R root.root /opt/cni/bin/* && chmod +x /opt/cni/bin/*"

    ssh root@v${I} "mkdir -p /opt/app/containerd-1.7.24-linux-amd64/bin && ln -snf /opt/app/containerd-1.7.24-linux-amd64 /opt/containerd"
    ...
    ssh root@v${I} "mkdir -p /opt/app/crictl-v1.31.1-linux-amd64/bin && ln -snf /opt/app/crictl-v1.31.1-linux-amd64 /opt/crictl"
    ...
    ssh root@v${I} "mkdir -p /opt/app/runc-v1.2.4-linux-amd64/sbin && ln -snf /opt/app/runc-v1.2.4-linux-amd64 /opt/runc"
...
tar xzvf release-v3.29.1.tgz
docker load --input ./release-v3.29.1/images/calico-cni.tar 
docker load --input ./release-v3.29.1/images/calico-kube-controllers.tar
docker load --input ./release-v3.29.1/images/calico-node.tar
vi ./release-v3.29.1/manifests/calico.yaml
```
...
```shell
kubectl apply -f ./release-v3.29.1/manifests/calico.yaml
```
...
```shell
cp ./release-v3.29.1/bin/calicoctl/calicoctl-linux-amd64 /usr/local/bin/kubectl-calico
```
