...
```shell
mv kubernetes kubernetes-1.29.10
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.29.10 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.29.10 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.29.10 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.29.10/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.29.10/node/bin/kubeadm /usr/local/bin/
```
...
tar xzvf release-v3.29.0.tgz
docker load --input ./release-v3.29.0/images/calico-cni.tar 
docker load --input ./release-v3.29.0/images/calico-kube-controllers.tar
docker load --input ./release-v3.29.0/images/calico-node.tar
vi ./release-v3.29.0/manifests/calico.yaml
```
...
```shell
kubectl apply -f ./release-v3.29.0/manifests/calico.yaml
```
...
```shell
cp ./release-v3.29.0/bin/calicoctl/calicoctl-linux-amd64 /usr/local/bin/kubectl-calico
```
