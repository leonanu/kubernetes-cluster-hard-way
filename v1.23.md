...
* v0
* [下载](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.23.md)
...
```shell
mv kubernetes kubernetes-1.23.6
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.23.6 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.23.6 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.23.6 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.23.6/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.23.6/node/bin/kubeadm /usr/local/bin/
```
...
ExecStart=/opt/etcd/etcd \\
  ...
  --client-cert-auth \\
  --listen-peer-urls=https://172.31.31.71.23.2 \\
  --initial-advertise-peer-urls=https://172.31.31.71.23.2 \\
  --listen-client-urls=https://172.31.31.71:2379,http://127.0.0.1:2379 \\
  --advertise-client-urls=https://172.31.31.71:2379 \\
  --initial-cluster-token=etcd-cluster-0 \\
  --initial-cluster="v1=https://172.31.31.71.23.2,v2=https://172.31.31.72:2380,v3=https://172.31.31.73:2380" \\
...
* [下载](https://github.com/cri-o/cri-o/releases/tag/v1.23.2#downloads)
...
* [下载](https://github.com/containerd/containerd/releases/download/v1.6.4/containerd-1.6.4-linux-amd64.tar.gz) Containerd

* [下载](https://github.com/containernetworking/plugins/releases/download/v1.1.1/cni-plugins-linux-amd64-v1.1.1.tgz) CNI-Plugins

* [下载](https://github.com/kubernetes-sigs/cri-tools/releases/download/v1.23.0/crictl-v1.23.0-linux-amd64.tar.gz) crictl

* [下载](https://github.com/opencontainers/runc/releases/download/v1.1.2/runc.amd64) RunC
...
```shell
tar xzvf containerd-1.6.4-linux-amd64.tar.gz -C ./containerd
```

```shell
tar xzvf crictl-v1.23.0-linux-amd64.tar.gz
```
...
for I in {1..6};do
    ...
    ssh root@v${I} "mkdir -p /opt/app/containerd-1.6.4-linux-amd64/bin && ln -snf /opt/app/containerd-1.6.4-linux-amd64 /opt/containerd"
    ...
    ssh root@v${I} "mkdir -p /opt/app/crictl-v1.23.0-linux-amd64/bin && ln -snf /opt/app/crictl-v1.23.0-linux-amd64 /opt/crictl"
    ...
    ssh root@v${I} "mkdir -p /opt/app/runc-v1.1.2-linux-amd64/sbin && ln -snf /opt/app/runc-v1.1.2-linux-amd64 /opt/runc"
...
* [下载](https://github.com/projectcalico/calicoctl/releases/download/v3.22.2/calicoctl) calicoctl
...
```shell
wget -O ingress-nginx-1.2.0.yaml https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.2.0/deploy/static/provider/baremetal/deploy.yaml
```
...
```shell
kubectl apply -f ./ingress-nginx-1.2.0.yaml
```
