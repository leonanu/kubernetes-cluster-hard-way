...
```shell
mv kubernetes kubernetes-1.23.10
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.23.10 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.23.10 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.23.10 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.23.10/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.23.10/node/bin/kubeadm /usr/local/bin/
```
...
ExecStart=/opt/etcd/etcd \\
  ...
  --client-cert-auth \\
  --listen-peer-urls=https://172.31.31.71.23.3 \\
  --initial-advertise-peer-urls=https://172.31.31.71.23.3 \\
  --listen-client-urls=https://172.31.31.71:2379,http://127.0.0.1:2379 \\
  --advertise-client-urls=https://172.31.31.71:2379 \\
  --initial-cluster-token=etcd-cluster-0 \\
  --initial-cluster="v1=https://172.31.31.71.23.3,v2=https://172.31.31.72:2380,v3=https://172.31.31.73:2380" \\
...
* [下载](https://github.com/cri-o/cri-o/releases/tag/v1.23.3#downloads)
