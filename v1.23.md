...
```shell
mv kubernetes kubernetes-1.23.5
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.23.5 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.23.5 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.23.5 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.23.5/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.23.5/node/bin/kubeadm /usr/local/bin/
```
...
* v0
* [下载](https://github.com/etcd-io/etcd/releases/tag/v3.5.3)
...
```shell
for I in {1..3};do
    rsync -avr /usr/local/src/etcd-v3.5.3-linux-amd64 root@v${I}:/opt/app/
    ssh root@v${I} "chown -R root.root /opt/app/etcd-v3.5.3-linux-amd64; ln -snf /opt/app/etcd-v3.5.3-linux-amd64 /opt/etcd"
done
```
...
* [下载](https://github.com/containerd/containerd/releases/download/v1.6.2/containerd-1.6.2-linux-amd64.tar.gz) Containerd
...
* [下载](https://github.com/opencontainers/runc/releases/download/v1.1.1/runc.amd64) RunC
...
```shell
tar xzvf containerd-1.6.2-linux-amd64.tar.gz -C ./containerd
```
...
for I in {1..6};do
    ...
    ssh root@v${I} "mkdir -p /opt/app/containerd-1.6.2-linux-amd64/bin && ln -snf /opt/app/containerd-1.6.2-linux-amd64 /opt/containerd"
    ...
    ssh root@v${I} "mkdir -p /opt/app/runc-v1.1.1-linux-amd64/sbin && ln -snf /opt/app/runc-v1.1.1-linux-amd64 /opt/runc"
...
```shell
wget -O ingress-nginx-1.1.3.yaml https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.1.3/deploy/static/provider/baremetal/deploy.yaml
```
...
```shell
kubectl apply -f ./ingress-nginx-1.1.3.yaml
```
...
```shell
tar xzvf ceph-csi-3.6.0.tar.gz
```

```shell
cd ./ceph-csi-3.6.0/deploy/cephfs/kubernetes
```
