...
```shell
mv kubernetes kubernetes-1.23.12
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.23.12 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.23.12 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.23.12 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.23.12/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.23.12/node/bin/kubeadm /usr/local/bin/
```
...
* v0
* [下载](https://github.com/etcd-io/etcd/releases/tag/v3.5.5.)
...
```shell
for I in {1..3};do
    rsync -avr /usr/local/src/etcd-v3.5.5.-linux-amd64 root@v${I}:/opt/app/
    ssh root@v${I} "chown -R root.root /opt/app/etcd-v3.5.5.-linux-amd64; ln -snf /opt/app/etcd-v3.5.5.-linux-amd64 /opt/etcd"
done
```
...
```shell
tar xzvf helm-v3.10.0-linux-amd64.tar.gz
```
...
```shell
wget -O ingress-nginx-1.3.1.yaml https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.3.1/deploy/static/provider/baremetal/deploy.yaml
```
...
```shell
tar xzvf ceph-csi-3.7.1.tar.gz
```

```shell
cd ./ceph-csi-3.7.1/deploy/cephfs/kubernetes
```
