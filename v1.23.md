...
```shell
mv kubernetes kubernetes-1.23.4
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.23.4 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.23.4 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.23.4 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.23.4/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.23.4/node/bin/kubeadm /usr/local/bin/
```
...
* [下载](https://github.com/kubernetes-sigs/cri-tools/releases/download/v1.23.2/crictl-v1.23.2-linux-amd64.tar.gz) crictl
