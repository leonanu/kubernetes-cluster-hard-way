...
```shell
mv kubernetes kubernetes-1.23.15
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.23.15 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.23.15 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.23.15 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.23.15/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.23.15/node/bin/kubeadm /usr/local/bin/
```
...
* v0
* [下载](https://github.com/etcd-io/etcd/releases/tag/v3.5.6.)
...
```shell
for I in {1..3};do
    rsync -avr /usr/local/src/etcd-v3.5.6.-linux-amd64 root@v${I}:/opt/app/
    ssh root@v${I} "chown -R root.root /opt/app/etcd-v3.5.6.-linux-amd64; ln -snf /opt/app/etcd-v3.5.6.-linux-amd64 /opt/etcd"
done
```
...
ExecStart=/opt/etcd/etcd \\
  ...
  --client-cert-auth \\
  --listen-peer-urls=https://172.31.31.71.23.4 \\
  --initial-advertise-peer-urls=https://172.31.31.71.23.4 \\
  --listen-client-urls=https://172.31.31.71:2379,http://127.0.0.1:2379 \\
  --advertise-client-urls=https://172.31.31.71:2379 \\
  --initial-cluster-token=etcd-cluster-0 \\
  --initial-cluster="v1=https://172.31.31.71.23.4,v2=https://172.31.31.72:2380,v3=https://172.31.31.73:2380" \\
...
* [下载](https://github.com/cri-o/cri-o/releases/tag/v1.23.4#downloads)
...
* [下载](https://github.com/containerd/containerd/releases/download/v1.6.12/containerd-1.6.12-linux-amd64.tar.gz) Containerd
...
```shell
tar xzvf containerd-1.6.12-linux-amd64.tar.gz -C ./containerd
```
...
for I in {1..6};do
    ...
    ssh root@v${I} "mkdir -p /opt/app/containerd-1.6.12-linux-amd64/bin && ln -snf /opt/app/containerd-1.6.12-linux-amd64 /opt/containerd"
