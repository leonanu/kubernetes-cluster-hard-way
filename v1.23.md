...
* [下载](https://github.com/opencontainers/runc/releases/download/v1.1.0/runc.amd64) RunC
...
for I in {1..6};do
    ...
    ssh root@v${I} "mkdir -p /opt/app/runc-v1.1.0-linux-amd64/sbin && ln -snf /opt/app/runc-v1.1.0-linux-amd64 /opt/runc"
...
```shell
wget -O ingress-nginx-1.1.2.yaml https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.1.2/deploy/static/provider/baremetal/deploy.yaml
```
...
```shell
kubectl apply -f ./ingress-nginx-1.1.2.yaml
```
...
```shell
tar xzvf ceph-csi-3.5.1.tar.gz
```
