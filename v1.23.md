...
```shell
mv kubernetes kubernetes-1.23.8
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.23.8 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.23.8 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.23.8 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.23.8/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.23.8/node/bin/kubeadm /usr/local/bin/
```
...
```shell
wget -O ingress-nginx-1.2.1.yaml https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.2.1/deploy/static/provider/baremetal/deploy.yaml
```
...
```shell
kubectl apply -f ./ingress-nginx-1.2.1.yaml
```
