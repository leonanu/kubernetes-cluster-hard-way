...
```shell
mv kubernetes kubernetes-1.23.7
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.23.7 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.23.7 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.23.7 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.23.7/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.23.7/node/bin/kubeadm /usr/local/bin/
```
...
* [下载](https://github.com/containerd/containerd/releases/download/v1.6.5/containerd-1.6.5-linux-amd64.tar.gz) Containerd
...
```shell
tar xzvf containerd-1.6.5-linux-amd64.tar.gz -C ./containerd
```
...
for I in {1..6};do
    ...
    ssh root@v${I} "mkdir -p /opt/app/containerd-1.6.5-linux-amd64/bin && ln -snf /opt/app/containerd-1.6.5-linux-amd64 /opt/containerd"
