...
```shell
mv kubernetes kubernetes-1.24.1
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.24.1 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.24.1 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.24.1 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.24.1/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.24.1/node/bin/kubeadm /usr/local/bin/
```
...
ExecStart=/opt/etcd/etcd \\
  ...
  --client-cert-auth \\
  --listen-peer-urls=https://172.31.31.71.24.1 \\
  --initial-advertise-peer-urls=https://172.31.31.71.24.1 \\
  --listen-client-urls=https://172.31.31.71:2379,http://127.0.0.1:2379 \\
  --advertise-client-urls=https://172.31.31.71:2379 \\
  --initial-cluster-token=etcd-cluster-0 \\
  --initial-cluster="v1=https://172.31.31.71.24.1,v2=https://172.31.31.72:2380,v3=https://172.31.31.73:2380" \\
...
* [下载](https://github.com/cri-o/cri-o/releases/tag/v1.24.1#downloads)
...
* [下载](https://github.com/kubernetes-sigs/cri-tools/releases/download/v1.24.2/crictl-v1.24.2-linux-amd64.tar.gz) crictl
