...
```shell
mv kubernetes kubernetes-1.24.3
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.24.3 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.24.3 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.24.3 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.24.3/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.24.3/node/bin/kubeadm /usr/local/bin/
```
...
* [下载](https://github.com/cri-o/cri-o/releases/tag/v1.24.2#downloads)
...
```shell
tar xzvf crictl-v1.24.2-linux-amd64.tar.gz
```
...
for I in {1..6};do
    ...
    ssh root@v${I} "mkdir -p /opt/app/crictl-v1.24.2-linux-amd64/bin && ln -snf /opt/app/crictl-v1.24.2-linux-amd64 /opt/crictl"
