...
* [下载](https://github.com/containerd/containerd/releases/download/v1.7.10/containerd-1.7.10-linux-amd64.tar.gz) Containerd
...
```shell
tar xzvf containerd-1.7.10-linux-amd64.tar.gz -C ./containerd
```
...
for I in {1..6};do
    ...
    ssh root@v${I} "mkdir -p /opt/app/containerd-1.7.10-linux-amd64/bin && ln -snf /opt/app/containerd-1.7.10-linux-amd64 /opt/containerd"
...
tar xzvf release-v3.26.4.tgz
docker load --input ./release-v3.26.4/images/calico-cni.tar 
docker load --input ./release-v3.26.4/images/calico-kube-controllers.tar
docker load --input ./release-v3.26.4/images/calico-node.tar
...
```shell
kubectl apply -f ./release-v3.26.4/manifests/calico.yaml
```
...
```shell
cp ./release-v3.26.4/bin/calicoctl/calicoctl-linux-amd64 /usr/local/bin/kubectl-calico
```
