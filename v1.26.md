...
* [下载](https://github.com/containerd/containerd/releases/download/v1.7.13/containerd-1.7.13-linux-amd64.tar.gz) Containerd
...
```shell
tar xzvf containerd-1.7.13-linux-amd64.tar.gz -C ./containerd
```
...
for I in {1..6};do
    ...
    ssh root@v${I} "mkdir -p /opt/app/containerd-1.7.13-linux-amd64/bin && ln -snf /opt/app/containerd-1.7.13-linux-amd64 /opt/containerd"
...
```shell
wget -O ingress-nginx-1.9.6.yaml https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.9.6/deploy/static/provider/baremetal/deploy.yaml
```
...
```shell
kubectl apply -f ./ingress-nginx-1.9.6.yaml
```
