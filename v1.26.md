...
* v0
* [下载](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.26.md)
...
```shell
mv kubernetes kubernetes-1.26.14
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.26.14 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.26.14 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.26.14 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.26.14/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.26.14/node/bin/kubeadm /usr/local/bin/
```
...
* v0
* [下载](https://github.com/etcd-io/etcd/releases/tag/v3.5.12)
...
```shell
for I in {1..3};do
    rsync -avr /usr/local/src/etcd-v3.5.12-linux-amd64 root@v${I}:/opt/app/
    ssh root@v${I} "chown -R root.root /opt/app/etcd-v3.5.12-linux-amd64; ln -snf /opt/app/etcd-v3.5.12-linux-amd64 /opt/etcd"
done
```
...
* [下载](https://github.com/cri-o/cri-o/releases/tag/v1.25.1#downloads)
...
* [下载](https://github.com/containerd/containerd/releases/download/v1.7.14/containerd-1.7.14-linux-amd64.tar.gz) Containerd

* [下载](https://github.com/containernetworking/plugins/releases/download/v1.4.0/cni-plugins-linux-amd64-v1.4.0.tgz) CNI-Plugins

* [下载](https://github.com/kubernetes-sigs/cri-tools/releases/download/v1.26.1/crictl-v1.26.1-linux-amd64.tar.gz) crictl

* [下载](https://github.com/opencontainers/runc/releases/download/v1.1.12/runc.amd64) RunC
...
```shell
tar xzvf containerd-1.7.14-linux-amd64.tar.gz -C ./containerd
```

```shell
tar xzvf crictl-v1.26.1-linux-amd64.tar.gz
```
...
for I in {1..6};do
    ...
    ssh root@v${I} "mkdir -p /opt/app/containerd-1.7.14-linux-amd64/bin && ln -snf /opt/app/containerd-1.7.14-linux-amd64 /opt/containerd"
    ...
    ssh root@v${I} "mkdir -p /opt/app/crictl-v1.26.1-linux-amd64/bin && ln -snf /opt/app/crictl-v1.26.1-linux-amd64 /opt/crictl"
    ...
    ssh root@v${I} "mkdir -p /opt/app/runc-v1.1.12-linux-amd64/sbin && ln -snf /opt/app/runc-v1.1.12-linux-amd64 /opt/runc"
...
```shell
wget https://github.com/projectcalico/calico/releases/download/v3.27.2/release-v3.27.2.tgz
tar xzvf release-v3.27.2.tgz
docker load --input ./release-v3.27.2/images/calico-cni.tar 
docker load --input ./release-v3.27.2/images/calico-kube-controllers.tar
docker load --input ./release-v3.27.2/images/calico-node.tar
vi ./release-v3.26.0/manifests/calico.yaml
```
...
```shell
kubectl apply -f ./release-v3.27.2/manifests/calico.yaml
```
...
```shell
cp ./release-v3.27.2/bin/calicoctl/calicoctl-linux-amd64 /usr/local/bin/kubectl-calico
```
...
```shell
tar xzvf helm-v3.14.3-linux-amd64.tar.gz
```
...
```shell
wget -O ingress-nginx-1.10.0.yaml https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.10.0/deploy/static/provider/baremetal/deploy.yaml
```
...
```shell
kubectl apply -f ./ingress-nginx-1.10.0.yaml
```
...
```shell
tar xzvf ceph-csi-3.10.2.tar.gz
```

```shell
cd ./ceph-csi-3.10.2/deploy/cephfs/kubernetes
```
