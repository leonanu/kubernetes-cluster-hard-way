...
```shell
mv kubernetes kubernetes-1.26.12
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.26.12 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.26.12 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.26.12 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.26.12/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.26.12/node/bin/kubeadm /usr/local/bin/
```
...
* v0
* [下载](https://github.com/etcd-io/etcd/releases/tag/v3.5.10)
...
```shell
for I in {1..3};do
    rsync -avr /usr/local/src/etcd-v3.5.10-linux-amd64 root@v${I}:/opt/app/
    ssh root@v${I} "chown -R root.root /opt/app/etcd-v3.5.10-linux-amd64; ln -snf /opt/app/etcd-v3.5.10-linux-amd64 /opt/etcd"
done
```
...
* [下载](https://github.com/containerd/containerd/releases/download/v1.7.11/containerd-1.7.11-linux-amd64.tar.gz) Containerd
...
* [下载](https://github.com/opencontainers/runc/releases/download/v1.1.11/runc.amd64) RunC
...
```shell
tar xzvf containerd-1.7.11-linux-amd64.tar.gz -C ./containerd
```
...
for I in {1..6};do
    ...
    ssh root@v${I} "mkdir -p /opt/app/containerd-1.7.11-linux-amd64/bin && ln -snf /opt/app/containerd-1.7.11-linux-amd64 /opt/containerd"
    ...
    ssh root@v${I} "mkdir -p /opt/app/runc-v1.1.11-linux-amd64/sbin && ln -snf /opt/app/runc-v1.1.11-linux-amd64 /opt/runc"
...
tar xzvf release-v3.27.0.tgz
docker load --input ./release-v3.27.0/images/calico-cni.tar 
docker load --input ./release-v3.27.0/images/calico-kube-controllers.tar
docker load --input ./release-v3.27.0/images/calico-node.tar
...
```shell
kubectl apply -f ./release-v3.27.0/manifests/calico.yaml
```
...
```shell
cp ./release-v3.27.0/bin/calicoctl/calicoctl-linux-amd64 /usr/local/bin/kubectl-calico
```
...
```shell
wget -O ingress-nginx-1.9.5.yaml https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.9.5/deploy/static/provider/baremetal/deploy.yaml
```
...
```shell
kubectl apply -f ./ingress-nginx-1.9.5.yaml
```
...
```shell
tar xzvf ceph-csi-3.10.1.tar.gz
```

```shell
cd ./ceph-csi-3.10.1/deploy/cephfs/kubernetes
```
