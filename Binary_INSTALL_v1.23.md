...
* v0
1. 作为 K8S API Server 和 Ingress 外部流量负载均衡，在生产环境中应部署至少两台主机，并安装配置 KeepAlived 实现高可用；

2. [下载](https://github.com/cloudflare/cfssl/releases/tag/v1.6.1) CloudFlare cfssl 证书签发工具 (cfssl / cfssjson)；
- v1 - v6
1. /var 分区容量足够大；
2. 配置 /etc/hosts 或 DNS 服务，保证所有节点均可通过主机名、域名访问；
3. 配置 systemd-timesyncd 或 NTP 时间同步；
4. 禁用 iptables；
5. v0 能够以 root 身份通过 SSH 密钥直接登录其他主机；
...
浏览器打开 http://172.31.31.70:9090/ha-status 。此时所有 backend 均不可用，因为此时尚未部署 Kube API Server 和 Nginx Ingress。
...
* v0
* [下载](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.23.md#downloads-for-v1231)
...
```shell
mv kubernetes kubernetes-1.23.1
```

```shell
# K8S 组件放置于 /opt/app/kubernetes-1.23.1 并软链至 /opt/kubernetes
for I in {1..6};do
    ssh root@v${I} "mkdir -p /opt/app"
    rsync -avr /usr/local/src/kubernetes-1.23.1 root@v${I}:/opt/app/
    ssh root@v${I} "ln -snf  /opt/app/kubernetes-1.23.1 /opt/kubernetes"
done
```
...
```shell
cp /usr/local/src/kubernetes-1.23.1/client/bin/kubectl /usr/local/bin/
```

```shell
cp /usr/local/src/kubernetes-1.23.1/node/bin/kubeadm /usr/local/bin/
```
...
* [下载](https://github.com/cri-o/cri-o/releases/tag/v1.23.0#downloads)
...
```shell
tar xzvf containerd-1.5.8-linux-amd64.tar.gz -C ./containerd
```
...
for I in {1..6};do
    ...
    ssh root@v${I} "mkdir -p /opt/app/containerd-1.5.8-linux-amd64/bin && ln -snf /opt/app/containerd-1.5.8-linux-amd64 /opt/containerd"
    ...
    rsync -avr /usr/local/src/runc.amd64.1.0.3 root@v${I}:/opt/runc/sbin/
    ssh root@v${I} "chown -R root.root /opt/runc/sbin/* && chmod +x /opt/runc/sbin/* && mv /opt/runc/sbin/runc.amd64.1.0.3 /opt/runc/sbin/runc"
...
```shell
wget -O ingress-nginx-1.1.0.yaml https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.1.0/deploy/static/provider/baremetal/deploy.yaml
```
...
```shell
kubectl apply -f ./ingress-nginx-1.1.0.yaml
```
...
```shell
tar xzvf ceph-csi-3.4.0.tar.gz
```
